...
def start_conversation(request, tool_id):
    ...
    # PostHog: Capture conversation_started event
    if settings.POSTHOG_API_KEY and request.user.is_authenticated:
        posthog.capture(
            request.user.email, # Distinct ID for the user
            'conversation_started',
            properties={
                'tool_id': str(tool.id), # Ensure ID is a string if it's UUID
                'tool_name': tool.name,
                'tool_api_type': tool.api_type,
                'conversation_id': str(conversation.id) # Ensure ID is a string
            }
        )
